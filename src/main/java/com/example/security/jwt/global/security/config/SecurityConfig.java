...
@EnableMethodSecurity // @PreAuthorize 어노테이션 사용을 위해 선언 [메서드수준에서 권한 검사를 수행]
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity httpSecurity, CustomJwtFilter customJwtFilter) throws Exception {
        httpSecurity
                ...
                .addFilterBefore(corsFilter, UsernamePasswordAuthenticationFilter.class) //   CORS(Cross-Origin Resource Sharing) 정책을 처리할 수 있도록 설정합니다.
                ...
                // H2 데이터베이스 콘솔과 같은 iframe을 사용하는 페이지 허용
                .and()
                .headers()
                .frameOptions()
                .sameOrigin()
